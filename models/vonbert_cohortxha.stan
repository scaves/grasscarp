data {
  int<lower = 0>              nobs;        // Number of observations
  int<lower = 0>              ngroups;     // number of groups
  int<lower = 0>              group[nobs]; // Group ID
  row_vector<lower = 0>[nobs] age;         // Age of fish
  row_vector<lower = 0>[nobs] length;      // Length of fish
  row_vector[nobs]            x;           // Covariate
  // Priors passed as data
  real p_linf;
  real p_linfsd;
  real p_k;
  real p_ksd;
  real p_t0;
  real p_t0sd;
  real p_sigma;
}
parameters {
  real<lower = 0>             sigma;   // Observation error
  row_vector[ngroups]         linf;    // Lognormal cohort linf
  row_vector[ngroups]         k;       // Lognormal cohort k
  row_vector[ngroups]         lt0;     // Lognormal cohort t0
  real linf_mu;    // Global linf
  real k_mu;       // Global k
  real t0_mu;      // Global t0 
  real blinf;
  real bk;  
}
transformed parameters {
  row_vector[nobs]  Linf;      // Asymptotic length
  row_vector[nobs]  K;         // Brody growth coefficient
  row_vector[nobs]  t0;        // Age at length 0
  
  for(i in 1:nobs){
    Linf[i] = exp(linf[group[i]] + blinf*x[i]);
    K[i] = exp(k[group[i]] + bk*x[i]);
    t0[i] = exp(lt0[group[i]]) - 10.0;
  }

}
model {
  row_vector[nobs] y;
  
  // Length as expectation of VBGF
  y = Linf[group] .* (1 - exp( -K[group] .* (age - t0[group])));
  
  linf_mu ~ normal(p_linf, p_linfsd);
  k_mu ~ normal(p_k, p_ksd);
  t0_mu ~ normal(p_t0, p_t0sd);
  
  for(i in 1:ngroups){
    linf ~ normal(linf_mu, 1);
    k ~ normal(k_mu, 1);
    lt0 ~ normal(t0_mu, 1);    
  }
  
  blinf ~ normal(0, 1);
  bk ~ normal(0, 1);
  
  //Process error
  sigma ~ exponential(1/p_sigma);
  
  // Likelihood
  length ~ normal(y, sigma);

}
