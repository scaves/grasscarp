...
transformed parameters {
  ...
  Linf = //theoretical maximum length
    exp(mu_beta_cor[1, fishID] + (b0_linf + (bh_linf*hydrilla[obs])));
  K = // growth coefficient 
    exp(mu_beta_cor[2, fishID] + (b0_k + (bh_k*hydrilla[obs])));
  t0 = // hypothetical age at which fish's size = 0
    mu_beta_cor[3, fishID] + (b0_t0);
}
model{
  row_vector[nobs] y;
